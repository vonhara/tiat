...
class Route extends Request {
	...
	protected function _setNamespaceDirectory(string $key) : bool {
		if(! empty($key)):
			...
			if(strtolower($key) !== self::defaultNamespace):
				// Do NOT use namespace if it's a "default"
				$path = PATH_APPS . ucfirst($key) . DIRECTORY_SEPARATOR;
			else:
				$path = PATH_APPS;
			endif;
	...
	protected function _setModule(string $key) : bool {
		if($this->_checkName($key)):
			...
			if($this->_setModuleDirectory($key)):
				return $this->_attachModuleToNamespace($key);
			endif;
		elseif($this->_nameModule === self::defaultModule):
			return TRUE;
		endif;
	...
	protected function _setModuleDirectory(string $key) : bool {
		if(! empty($key)):
			// Set module directory
			$this->_dirModule =
				$this->_dirNamespace . ucfirst($this->_nameModule) . DIRECTORY_SEPARATOR . 'Controller' .
				DIRECTORY_SEPARATOR;
			
			// Set include path
			ini_set('include_path', ini_get('include_path') . PATH_SEPARATOR . $this->_dirNamespace . 'Controller' .
			                        DIRECTORY_SEPARATOR);
			
			// Set path for TEMPLATES also
			if(! defined('PATH_TEMPLATES')):
				define('PATH_TEMPLATES',
				       $this->_dirNamespace . ucfirst($this->_nameModule) . DIRECTORY_SEPARATOR . 'Template' .
				       DIRECTORY_SEPARATOR);
			endif;
		endif;
	 ...
	 * @return bool
	 */
	protected function _attachModuleToNamespace(string $key) : bool {
		// Set basic namespace module info (needed sometimes)
	...
	public function getController() : Action {
		if(! empty($this->_dirModule) && ! empty($this->_fileController) &&
		   file_exists($file = $this->_dirModule . $this->_fileController)):
			require_once( $file );
